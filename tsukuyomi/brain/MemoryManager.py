...
class MemoryManager:
    ...
    def __init__(self, agent_id: str):
        ...
        # FIX: Keyword indexing for O(1) lookup instead of O(N) string search
    ...
    def ingest_tick(self, tick_state: core_pb2.TickState):
        ...
        for res in tick_state.resolutions:
            ...
            # FIX: Add to keyword index for O(1) lookup
            topics = memory_entry.get("topics", [])
            for topic in topics:
    ...
    def query_by_relevance(self, topics: List[str], limit: int = 5) -> List[Dict]:
        ...
        FIX: Use keyword_index for O(1) lookup instead of O(N) string search.
        """
        if not topics:
            # If no topics, return recent memories
            return self.query_recent(limit)

        # Use keyword index to find candidate memory indices
        candidate_indices = set()
        for topic in topics:
            topic_lower = topic.lower()
            if topic_lower in self.keyword_index:
                candidate_indices.update(self.keyword_index[topic_lower])

        # If no candidates found via index, return recent memories
        if not candidate_indices:
            return self.query_recent(limit)

        # Score only the candidate memories (much faster than O(N))
        scored = []
        for idx in candidate_indices:
            ...
            # Count topic matches
            topic_match = sum(1 for t in topics if t.lower() in mem_topics)
