...
This module implements consistency checks for agent reasoning. It validates
that reasoning is consistent with personality traits, beliefs, and needs.

Core Components:
- ReasoningValidator: Main validation orchestrator
- ReasoningViolation: A detected inconsistency
- ValidationResult: Complete validation result
- TraitChecker: Trait-specific consistency checks
- BeliefChecker: Belief-action consistency checks
- NeedsChecker: Needs-response consistency checks
"""
...
class ViolationSeverity(Enum):
    ...
    LOW = 0.1       # Minor inconsistency, acceptable
    MEDIUM = 0.3    # Moderate inconsistency, should be logged
    HIGH = 0.5      # Significant inconsistency, should be flagged
    CRITICAL = 0.8  # Severe inconsistency, should be rejected
...
class ReasoningViolation:
    ...
    Captures what was expected vs what actually happened, allowing
    for debugging and improvement of agent behavior.
    """
...
class ValidationResult:
    ...
    Contains all violations found and aggregate metrics.
    """
    ...
    def add_violation(self, violation: ReasoningViolation) -> None:
        """Add a violation and update metrics."""
...
class PersonalitySnapshot:
    ...
    Contains the Big Five traits and behavioral patterns that
    influence validation rules.
    """
    ...
    # Core values and biases
    core_values: List[str] = field(default_factory=list)
    biases: List[str] = field(default_factory=list)

    # Vocabulary level
    vocabulary_level: str = "medium"  # "simple", "medium", "educated", "formal"
...
class BeliefSnapshot:
    ...
    Contains current stances on relevant topics.
    """
...
class NeedsSnapshot:
    ...
    Contains current need levels and urgency.
    """

    # Need type -> (level, is_critical)
...
class TraitChecker:
    ...
    This checker ensures that agent behavior is consistent with
    their Big Five personality profile.
    """
    ...
    def check_trait_consistency(
        ...
        Args:
            ...
            heat_level: Conversation tension level
    ...
    def _check_extraversion(
        ...
        # Introvert initiating public speech/shout
        if self.personality.extraversion < -0.3:
            ...
            # Introvert initiating conversation unprompted
            if action == "SPEAK" and not params.get("response_to"):
                # Check if reasoning justifies this
        ...
        # Extravert avoiding social interaction
    ...
    def _check_agreeableness(
        ...
        # Highly disagreeable person being overly cooperative
        if self.personality.agreeableness < -0.5:
            reasoning_lower = reasoning.lower()
    ...
    def _check_neuroticism(
        ...
        # High neuroticism with calm response under pressure
        ...
        # Low neuroticism showing extreme emotional reaction
    ...
    def _check_conscientiousness(
        ...
        # High conscientiousness doing spontaneous risky actions
        ...
        # Low conscientiousness with overly planned behavior
    ...
    def _check_vocabulary(self, reasoning: str) -> List[ReasoningViolation]:
        ...
        if self.personality.vocabulary_level == "simple":
            # Count complex words (simplified check)
            complex_patterns = [
...
class BeliefChecker:
    ...
    This checker ensures that agent behavior is consistent with
    their belief system and stances on topics.
    """
    ...
    def check_belief_consistency(
        ...
        Args:
            ...
            params: Action parameters
            reasoning: The reasoning chain
            personality_drivers: List of personality drivers
        ...
        # Check for belief-action contradictions
    ...
    def _check_belief_action_consistency(
        ...
        # Extract relevant belief topics from action
        for topic, (stance, confidence) in self.beliefs.stances.items():
            # Check if the action relates to this topic
            if self._action_relates_to_topic(action, params, topic):
                # Check if reasoning contradicts stance
    ...
    def _check_stance_reversal(
        ...
        # Extract belief influences from personality drivers
        for driver in personality_drivers:
            if driver.startswith("belief:"):
                # Parse belief influence
                # Format: "belief: topic (stance): influence"
                parts = driver.split(":", 2)
                if len(parts) >= 3:
                    topic_part = parts[1].strip()
                    # Check if this contradicts current stance
                    for topic, (stance, confidence) in self.beliefs.stances.items():
                        if topic in topic_part.lower():
                            # Check if reasoning shows reversal
...
class NeedsChecker:
    ...
    This checker ensures that agents don't ignore their critical needs
    ...
    def check_needs_consistency(
        ...
        # Map needs to actions that address them
        need_action_map = {
            "hunger": ["COLLECT", "INTERACT", "MOVE"],  # Food-related actions
            "fatigue": ["IDLE", "REFLECT", "SLEEP"],     # Rest actions
            "boredom": ["MOVE", "INTERACT", "EMOTE", "EXPLORE"],  # Stimulation
            "social": ["EMOTE", "INTERACT", "SPEAK"]    # Social interaction
        }
        ...
        for need in critical_needs:
            ...
            if action not in addressing_actions:
                # Check if reasoning acknowledges the need
                if need.lower() not in reasoning.lower():
                    ...
                    # Need was acknowledged but not addressed - lower severity
                    violations.append(ReasoningViolation(
                        violation_type=ViolationType.NEEDS_IGNORED,
...
class ReasoningValidator:
    ...
    Coordinates multiple consistency checkers to validate that
    reasoning is coherent with personality, beliefs, and needs.

    Usage:
        validator = ReasoningValidator(personality, beliefs, needs)
    ...
    def __init__(
        ...
        """
        Initialize the validator with agent profiles.

        Args:
            personality: Personality snapshot for trait checks
            beliefs: Belief snapshot for belief consistency
            needs: Needs snapshot for need addressing
        """
    ...
    def update_profiles(
        ...
        Args:
            personality: New personality snapshot
            beliefs: New belief snapshot
            needs: New needs snapshot
        """
        if personality:
            self.personality = personality
    ...
    def validate_decision_record(self, record: 'DecisionRecord') -> ValidationResult:
        ...
        This is a convenience method for validating the full record
        including all context.

        Args:
            record: The DecisionRecord to validate
