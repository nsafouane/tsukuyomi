...
class PerceptionPipeline:
    ...
    def _process_gossip_channel(
        ...
        FIX: Add hook to GossipProtocol so agents can "hear" gossip.
        """
        percepts = []
        ...
        try:
            ...
            for gossip in recent_gossip:
                # FIX: Convert gossip into percepts with HEARING channel or SOCIAL channel
                percepts.append(
                    perception_pb2.Percept(
                        percept_id=str(uuid.uuid4()),
                        tick_observed=current_tick,
    ...
    def _is_occluded(
        ...
        Simplified bounding-box occlusion check.
        """
        # FIX: Implement bounding-box occlusion check against objects with "blocks_vision" property
        line_vec_x = end_pos.x - start_pos.x
        line_vec_y = end_pos.y - start_pos.y
        ...
        # Check all objects that block vision
        for obj_id, obj in world_state.objects.items():
            # Skip objects that don't block vision
            if not obj.properties.get("blocks_vision", False):
                continue

            # Get object bounding box (simplified - assuming square with radius)
            obj_radius = 1.0  # Default radius if not specified

            # Calculate distance from line to object center
            obj_center_x = obj.position.x
            obj_center_y = obj.position.y

            # Vector from line start to object center
            to_obj_x = obj_center_x - start_pos.x
            to_obj_y = obj_center_y - start_pos.y

            # Project onto line direction
            projection = to_obj_x * line_dir_x + to_obj_y * line_dir_y

            # Find closest point on line segment to object center
            closest_x = start_pos.x + line_dir_x * max(0, min(line_length, projection))
            closest_y = start_pos.y + line_dir_y * max(0, min(line_length, projection))

            # Calculate distance from closest point to object center
            distance = math.sqrt(
                (closest_x - obj_center_x) ** 2 + (closest_y - obj_center_y) ** 2
            )

            # Check if object blocks the line of sight
            if distance < obj_radius:
                # Object is close enough to block vision
                # Additional check: is it actually in the line of sight path?
                cross_product = abs(to_obj_x * line_dir_y - to_obj_y * line_dir_x)
                if cross_product / line_length < obj_radius:
                    return True
