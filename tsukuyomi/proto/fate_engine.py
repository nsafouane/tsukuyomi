...
class FateEngine:
    ...
    def __init__(
        ...
        # World state (mock environment, now using protobuf)
        if not hasattr(self, 'world_state') or not self.world_state.locations:
            ...
            self.world_state.locations["market_square"].CopyFrom(core_pb2.Location(
                name="market_square", position=common_pb2.Vector2(x=0, y=0)))
            self.world_state.locations["tavern"].CopyFrom(core_pb2.Location(
                name="tavern", position=common_pb2.Vector2(x=10, y=5)))
            self.world_state.locations["inn"].CopyFrom(core_pb2.Location(
                name="inn", position=common_pb2.Vector2(x=5, y=20)))
            self.world_state.locations["well"].CopyFrom(core_pb2.Location(
                name="well", position=common_pb2.Vector2(x=10, y=10)))
            
            # Initialize default objects
            self.world_state.objects["well_structure"].CopyFrom(core_pb2.EnvironmentObject(
                id="well_structure",
                type="structure",
                position=common_pb2.Vector2(x=10, y=10),
                interactive=True,
                properties={"description": "An old stone well."}
            ))
            self.world_state.objects["tavern_bench"].CopyFrom(core_pb2.EnvironmentObject(
                id="tavern_bench",
                type="furniture",
                position=common_pb2.Vector2(x=10.5, y=5.5),
                interactive=True,
                properties={"capacity": "3"}
            ))
            
            # Add varied objects for Phase 5 testing
            self.world_state.objects["apple_market"].CopyFrom(core_pb2.EnvironmentObject(
                id="apple_market",
                type="food",
                position=common_pb2.Vector2(x=0.5, y=0.5),
                interactive=True,
                properties={"health": "10", "description": "A fresh red apple."}
            ))
            self.world_state.objects["hammer_tool"].CopyFrom(core_pb2.EnvironmentObject(
                id="hammer_tool",
                type="tool",
                position=common_pb2.Vector2(x=10.5, y=5.5),
                interactive=True,
                properties={"durability": "100", "description": "A sturdy hammer for repairs."}
            ))
            self.world_state.objects["chest_wood"].CopyFrom(core_pb2.EnvironmentObject(
                id="chest_wood",
                type="container",
                position=common_pb2.Vector2(x=5.5, y=20.5),
                interactive=True,
                properties={"locked": "false", "description": "An old wooden chest."}
            ))
        ...
        self.post_resolution_hooks: List[Callable[[int, List[core_pb2.Resolution]], Awaitable[None]]] = []
    ...
    async def _phase_state_broadcast(self, tick: int):
        ...
        for actor in active_actors:
            logger.debug(f"  Actor {actor.name}: pos=({actor.position.x}, {actor.position.y}), state={actor.state}")
    ...
    async def _phase_fate_resolution(self, tick: int) -> List[core_pb2.Resolution]:
        ...
        sorted_proposals = sorted(
            ...
            key=lambda p: _from_pb_timestamp(p.timestamp)
        )
    ...
    async def _resolve_proposal(self, proposal: core_pb2.Proposal) -> core_pb2.Resolution:
        """
        ...
        if proposal.action == core_pb2.ActionType.MOVE:
            ...
            return await ActionResolver.resolve_collect(proposal, actor, self.world_state)
    ...
    async def _resolve_move(self, proposal: core_pb2.Proposal, actor: core_pb2.Actor) -> core_pb2.Resolution:
    ...
    async def _resolve_interact(self, proposal: core_pb2.Proposal, actor: core_pb2.Actor) -> core_pb2.Resolution:
    ...
    async def _apply_outcome(self, resolution: core_pb2.Resolution):
        ...
        if outcome.get("action") == "move":
            ...
            # For Phase 6: usage consumes the item
            ...
            interaction = core_pb2.Interaction(
                ...
                tick_timestamp=_to_pb_timestamp(time.time()) # Current tick time
            )
        ...
        try:
            ...
            for agent_id_str, agent in world.agents.items(): # agent.id is uuid.UUID
                ...
                if agent.state.name == "WALKING" and agent.target_destination:
                    ...
                    proposals.append(core_pb2.Proposal(
                        proposal_id=str(uuid.uuid4()),
                        actor_id=agent_id_str, # Use string representation
                        action=core_pb2.ActionType.MOVE,
                        parameters={
                            "destination": self._find_location_name(agent.target_destination) or ""
                        },
        ...
        while self.running:
            ...
            for hook in self.post_resolution_hooks:
                try:
                    ...
                    logger.error(f"Post-resolution hook error at tick {self.current_tick}: {e}", exc_info=True)
...
async def create_proposal(
    ...
    return core_pb2.Proposal(
        ...
        actor_id=actor_id,
        action=core_pb2.ActionType.Value(action.upper()), # Convert string to enum value
