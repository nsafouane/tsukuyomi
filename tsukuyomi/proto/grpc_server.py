...
class FateEngineServicer(fate_engine_service_pb2_grpc.FateEngineServiceServicer):
    ...
    def __init__(self, fate_engine: FateEngine):
        ...
        # Track subscribers with their settings
        # Each entry is a tuple: (queue, downsample_factor)
        self._tick_subscribers: list[tuple[asyncio.Queue, int]] = []
    ...
    async def _broadcast_tick_update(self, tick: int, resolutions: list):
        """Broadcast tick state to all subscribers, respecting their downsample factor."""
        ...
        async with self._subscriber_lock:
            dead_subscribers = []
            for queue, downsample_factor in self._tick_subscribers:
                # Apply downsample filter: send tick only if tick_number % factor == 0
                if downsample_factor <= 1 or (tick % downsample_factor == 0):
                    try:
                        queue.put_nowait(tick_state)
                    except asyncio.QueueFull:
                        # Queue is full, subscriber is too slow
                        logger.warning(f"Subscriber queue full (factor={downsample_factor}), dropping tick update")
                    except Exception as e:
                        logger.error(f"Error broadcasting to subscriber: {e}")
                        dead_subscribers.append((queue, downsample_factor))
            
            # Clean up dead subscribers
            for sub in dead_subscribers:
                if sub in self._tick_subscribers:
                    self._tick_subscribers.remove(sub)
    ...
    async def StreamTickUpdates(
        ...
        Supports tick downsampling for high-latency clients.
        """
        # Parse downsample factor from request (default: 1 = full 20 TPS)
        downsample_factor = request.tick_downsample_factor if request.tick_downsample_factor > 0 else 1

        # Validate downsample factor (must be positive integer)
        if downsample_factor < 1:
            downsample_factor = 1

        # Adjust queue size based on downsample factor
        # Higher downsample = smaller queue needed (fewer ticks per second)
        queue_size = max(10, 100 // downsample_factor)

        # Create a queue for this subscriber
        queue: asyncio.Queue[core_pb2.TickState] = asyncio.Queue(maxsize=queue_size)

        async with self._subscriber_lock:
            self._tick_subscribers.append((queue, downsample_factor))

        logger.info(f"New tick stream subscriber connected (factor={downsample_factor}, queue_size={queue_size}, total: {len(self._tick_subscribers)})")
        ...
        try:
            ...
            async with self._subscriber_lock:
                subscriber_tuple = (queue, downsample_factor)
                if subscriber_tuple in self._tick_subscribers:
                    self._tick_subscribers.remove(subscriber_tuple)
...
class GrpcServer:
    ...
    async def stop(self, grace: float = 5.0):
        ...
        if self.server:
            ...
            # Small delay to allow pending async tasks in the loop to settle
