...
class ConversationManager:
    ...
    def __init__(
        ...
        self.max_turns_per_speaker = max_turns_per_speaker
        self.min_silence_ticks = min_silence_ticks
        
        self.current_speaker: Optional[str] = None
        self.speaker_queue: List[str] = []
        self.turn_counts: Dict[str, int] = {p: 0 for p in participants}
        self.last_speaker: Optional[str] = None
        self.silence_ticks: int = 0
        self.total_turns: int = 0
        
        self.response_histories: Dict[str, ResponseHistory] = {
            p: ResponseHistory() for p in participants
        }
    ...
    def request_turn(self, agent_id: str, context: TurnContext) -> TurnResult:
        ...
        Args:
            agent_id: Agent requesting to speak
            context: Current conversation context
        
        Returns:
            TurnResult with decision
        """
        if agent_id not in self.participants:
            return TurnResult(
                decision=TurnDecision.SILENT,
                reason="Agent not a participant"
            )
        
        if self.current_speaker is None:
            return self._grant_turn(agent_id, "No current speaker")
        
        if self.current_speaker == agent_id:
            if self.turn_counts[agent_id] < self.max_turns_per_speaker:
                return self._grant_turn(agent_id, "Continuing turn")
            else:
                return TurnResult(
                    decision=TurnDecision.YIELD,
                    reason="Max turns reached"
                )
        
        if self.silence_ticks >= self.min_silence_ticks:
            return self._grant_turn(agent_id, "Silence threshold met")
        
        return TurnResult(
            decision=TurnDecision.YIELD,
            reason="Waiting for current speaker"
        )
    
    def _grant_turn(self, agent_id: str, reason: str) -> TurnResult:
        """Grant a turn to an agent."""
    ...
    def get_next_speaker(self) -> Optional[str]:
        ...
        eligible = [
            ...
            if self.turn_counts[p] < self.max_turns_per_speaker
        ]
        
        if not eligible:
            return None
        
        eligible.sort(key=lambda p: self.turn_counts[p])
        return eligible[0]
    ...
    def record_response(
        ...
        """Record a response for an agent."""
        if agent_id in self.response_histories:
...
def create_conversation_manager(
    ...
    **kwargs
    ...
    return ConversationManager(participants=participants, **kwargs)
